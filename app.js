...
// 1. Cập nhật đếm số lượng tài khoản (Tính cả QLTP)
function updateStatChips() {
  ...
  // Tính tổng User = Ngành Hàng + QLTP + Admin (1)
...
// Cập nhật hàm Render danh sách, gộp chung QLTP và Ngành Hàng
function renderMasterList(type) {
  ...
  if (!items.length && type !== 'users') { el.innerHTML = '<p style="padding:12px;color:#888;">Trống — import data hoặc thêm thủ công</p>'; return; }

  if (type === 'users') {
    const nhUsers = DB.get('nhUsers') || [];
    const qltpList = DB.get('qltpList') || [];
    let rows = `<tr style="background:#fff3cd;"><td style="padding:6px;font-weight:bold;">admin</td><td style="padding:6px;">Admin BHX</td><td style="padding:6px;font-weight:bold;color:#666;">Admin</td><td></td></tr>`;
...
// Hàm Thêm User xử lý cả Ngành hàng và QLTP
...
// Hàm Xóa User xử lý cho cả 2 loại
